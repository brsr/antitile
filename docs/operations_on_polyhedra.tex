...
polyhedra will be denoted $\Delta_{a,b}$, and on quadrilateral-faced polyhedra
$\Box_{a,b}$, where $a$ and $b$ are integers, $a > 0$, and $b \ge 0$.
...
integers, $x = a + bi$ where $i = \sqrt{-1}$. The master polygon is contained
...
to reduce $\Delta_{a,b}$ and $\Box_{a,b}$ into other operators, in
...
$\Delta_{a,a} = \Delta_{1,1}\Delta_{a,0}$, and possibly further (and similiarly
for $\Box_{a,b}$). In fact, $\Delta_{a,b}$ is reducible if $a \equiv b \mod 3$
(as $\Delta_{1,1}$ and some other $\Delta_{c,d}$), and $\Box_{a,b}$ is
reducible if $a \equiv b \mod 2$ (as $\Box_{1,1}$ and some other $\Box_{c,d}$).
...
the labeling of vertices uses the same letters VEF. This is somewhat confusing
since E and F no longer relate to edges or faces, so alternately the vertices
can be labeled ABC. If applied to a non-orientable polyhedron, \texttt{wythoff}
instead uses the orientable double cover of the polyhedron.
...
It is also capable of producing certain quasipolyhedra.
...
homeomorphism in graph theory: see \cite{ziegler} for context. Antiprism
...
For instance, the endpoint for any spherical hosohedron or spherical dihedron
is the spherical monohedron. It may be helpful to define the operator to return
a proper polyhedra. This ambiguity will not affect the rest of this text.
...
\section{Table of operator cohorts}
...
\caption{$p$-cohort of operators on orientable polyhedra}
\begin{tabular}[t]{ c c|p{1cm} c c p{2cm} }
\hline \hline
$x \in \mathcal{W}$ & $M_{x}$ & $@$ & $\$x@m \in \mathcal{P}$ & $M_{\$x@m}$
& Note
...
\begin{table}
\caption{$g$-cohort of operators on orientable polyhedra}
\begin{tabular}[t]{ c c|p{1cm} c c p{2cm} }
\hline \hline
$x \in \mathcal{W}$ & $M_{x}$ & $@$ & $\$x@m \in \mathcal{P}$ & $M_{\$x@m}$
& Note
...
\begin{table}%verify that the orientation is the same as in wythoff
\caption{$B$-cohort of operators on orientable polyhedra}
\begin{tabular}[t]{ c c|p{1cm} c c p{2cm} }
\hline \hline
$x \in \mathcal{W}$ & $M_{x}$ & $@$ & $\$x@m \in \mathcal{P}$ & $M_{\$x@m}$
& Note
...
\begin{table}
\caption{$M_n$-cohort of operators on polyhedra, $n=2k, k \ge 1$}
\begin{tabular}[t]{ c c|p{1cm} c c p{2cm} }
\hline \hline
$x \in \mathcal{W}$ & $M_{x}$ & $@$ & $\$x@m \in \mathcal{P}$ & $M_{\$x@m}$
& Note
...
\begin{table}
\caption{$M_n$/$m_n$-cohort of operators on polyhedra}
\begin{tabular}[t]{ c c|p{1cm} c c p{2cm} }
\hline \hline
$x \in \mathcal{W}$ & $M_{x}$ & $@$ & $\$x@m \in \mathcal{P}$ & $M_{\$x@m}$
& Note
...
\begin{table}
\caption{$m_n$-cohort of operators on polyhedra, $n=2k+1, k \ge 0$}
\begin{tabular}[t]{ c c|p{1cm} c c p{2cm} }
\hline \hline
$x \in \mathcal{W}$ & $M_{x}$ & $@$ & $\$x@m \in \mathcal{P}$ & $M_{\$x@m}$
& Note
